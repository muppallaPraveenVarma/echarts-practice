...
class ApiModelGenerator {
    constructor(collector) {
        ...
        this._referenceGenerator = new DeclarationReferenceGenerator_1.DeclarationReferenceGenerator(collector.packageJsonLookup, collector.workingPackage.name, collector.program, collector.typeChecker);
    }
    get apiModel() {
    ...
    _processDeclaration(astDeclaration, exportedName, parentApiItem) {
        ...
        const declarationMetadata = this._collector.fetchMetadata(astDeclaration);
        if (declarationMetadata.isAncillary) {
            // Skip ancillary declarations; we will process them with the main declaration
            return;
        }
        const releaseTag = declarationMetadata.effectiveReleaseTag;
    ...
    _processApiCallSignature(astDeclaration, exportedName, parentApiItem) {
        ...
        if (apiCallSignature === undefined) {
            ...
            const declarationMetadata = this._collector.fetchMetadata(astDeclaration);
            const docComment = declarationMetadata.tsdocComment;
            const releaseTag = declarationMetadata.effectiveReleaseTag;
